...
CREATE OR REPLACE PACKAGE BODY PKG_MAHOA AS
  ...
  -- Khop voi C#, dam bao 32 bytes de sua loi ORA-28234
  ...
  BEGIN
    ...
    -- Thay the UTL_I18N.RAW_TO_STRING de tranh loi phien ban
    v_decrypted_data := UTL_RAW.CAST_TO_VARCHAR2(v_decrypted_raw); -- Can quyen UTL_RAW
...
CREATE TABLESPACE TS_QUANLYTRASUA
    DATAFILE 'C:\ORACLE_12C\oradata\orcl\trasua_data_01.dbf'-- (DOI DUONG DAN NAY NEU CAN)
    SIZE 100M 
    REUSE
...
-- 2. & 3. Tao va luu cap khoa RSA (Mock keys)
DECLARE
    v_pub_key RAW(2000) := HEXTORAW('0123456789ABCDEF0123456789ABCDEF0123456789ABCDEF0123456789ABCDEF');
    v_priv_key RAW(2000) := HEXTORAW('FEDCBA9876543210FEDCBA9876543210FEDCBA9876543210FEDCBA9876543210');
BEGIN
    DELETE FROM KEY_STORAGE WHERE KeyType = 'RSA_SIGN_KEY';
    INSERT INTO KEY_STORAGE (KeyType, PublicKey, PrivateKey)
    VALUES ('RSA_SIGN_KEY', v_pub_key, v_priv_key);
    
    DELETE FROM KEY_STORAGE WHERE KeyType = 'RSA_ENCRYPT_KEY';
    INSERT INTO KEY_STORAGE (KeyType, PublicKey, PrivateKey)
...
CREATE OR REPLACE PACKAGE BODY PKG_CHUKYSO AS
  ...
  BEGIN
    -- Mock signature - trong thuc te can thay the bang giai phap that
    RETURN UTL_RAW.CAST_TO_RAW('MOCK_SIGNATURE_' || p_data);
  EXCEPTION
  ...
  BEGIN
    -- Mock verification - luon tra ve TRUE de demo
    -- Trong thuc te can thay the bang giai phap that
    RETURN TRUE;
  EXCEPTION
...
CREATE OR REPLACE PACKAGE BODY PKG_MAHOA_RSA AS
  ...
  BEGIN
    -- Mock encryption - chi tra ve raw version cua data
    RETURN UTL_RAW.CAST_TO_RAW(p_data_plain);
  END F_ENCRYPT_SDT;

  FUNCTION F_DECRYPT_SDT(p_data_encrypted IN RAW) RETURN VARCHAR2 AS
  BEGIN
    -- Mock decryption
    IF p_data_encrypted IS NULL THEN 
        RETURN '(Chua co SDT)';
    END IF;
    RETURN UTL_RAW.CAST_TO_VARCHAR2(p_data_encrypted);
  EXCEPTION
    WHEN OTHERS THEN
        RETURN '(Loi giai ma)';
  END F_DECRYPT_SDT;
...
CREATE OR REPLACE PACKAGE BODY PKG_MAHOA_HYBRID AS
  ...
  BEGIN
    -- Mock encryption - su dung AES tu package PKG_MAHOA
    p_data_encrypted := PKG_MAHOA.F_ENCRYPT(p_data_plain);
    p_key_encrypted := UTL_RAW.CAST_TO_RAW('MOCK_KEY');
  ...
  BEGIN
    ...
    -- Mock decryption - su dung AES tu package PKG_MAHOA
    RETURN PKG_MAHOA.F_DECRYPT(p_data_encrypted);
...
BEGIN
    ...
    -- Insert khong dung CHAR_TO_LABEL
    INSERT INTO SANPHAM (MaSP, TenSP, DonGia, LoaiSP, SoLuongTon, OLS_LABEL)
    VALUES (
        p_MaSP, 
        p_TenSP, 
        p_DonGia, 
        p_LoaiSP,
        p_SoLuongTon,
        v_ols_label  -- Chi luu chuoi, khong dung CHAR_TO_LABEL
    );
    COMMIT;
EXCEPTION
    WHEN OTHERS THEN
...
-- 10. Tao Function kiem tra quyen truy cap (Thay the OLS)
CREATE OR REPLACE FUNCTION F_Check_Product_Access(
    p_ols_label IN VARCHAR2,
...
BEGIN
    ...
    -- Nhan vien ban hang chi xem duoc PUBLIC
    IF p_user_role = 'NhanVien' AND p_ols_label LIKE 'PUB:%' THEN
        RETURN 'ALLOW';
    END IF;
    
    -- Quan ly kho xem duoc ca PUBLIC va INTERNAL
    IF p_user_role = 'QuanLyKho' AND (p_ols_label LIKE 'PUB:%' OR p_ols_label LIKE 'INT:%') THEN
        RETURN 'ALLOW';
    END IF;
...
-- 0. Kiem tra va sua cot OLS_LABEL
DECLARE
    v_column_exists NUMBER;
    v_data_type VARCHAR2(100);
BEGIN
    -- Kiem tra xem cot OLS_LABEL da ton tai chua
    SELECT COUNT(*) INTO v_column_exists
    FROM USER_TAB_COLUMNS
    WHERE TABLE_NAME = 'SANPHAM' AND COLUMN_NAME = 'OLS_LABEL';
    
    -- Neu cot ton tai, kiem tra kieu du lieu
    IF v_column_exists > 0 THEN
        SELECT DATA_TYPE INTO v_data_type
        FROM USER_TAB_COLUMNS
        WHERE TABLE_NAME = 'SANPHAM' AND COLUMN_NAME = 'OLS_LABEL';
        
        -- Neu la kieu NUMBER, xoa va tao lai kieu VARCHAR2
        IF v_data_type = 'NUMBER' THEN
            EXECUTE IMMEDIATE 'ALTER TABLE SANPHAM DROP COLUMN OLS_LABEL';
            EXECUTE IMMEDIATE 'ALTER TABLE SANPHAM ADD OLS_LABEL VARCHAR2(50)';
        END IF;
    ELSE
...
-- 1. Gan nhan cho du lieu hien co
-- Tra sua (TS...) la 'Public' (ca Kho va Ban hang deu thay)
UPDATE SANPHAM
SET OLS_LABEL = 'PUB:KHO,BH'
WHERE MaSP LIKE 'TS%';

-- Topping (TP...) la 'Internal' (chi Kho thay)
UPDATE SANPHAM
SET OLS_LABEL = 'INT:KHO'
WHERE MaSP LIKE 'TP%';

COMMIT;

-- 2. Cap nhat lai SP Them San pham de dam bao dung kieu du lieu
CREATE OR REPLACE PROCEDURE SP_SanPham_Insert
(
    p_MaSP IN SANPHAM.MaSP%TYPE,
    p_TenSP IN SANPHAM.TenSP%TYPE,
...
BEGIN
    ...
    -- Insert voi OLS_LABEL la VARCHAR2
    INSERT INTO SANPHAM (MaSP, TenSP, DonGia, LoaiSP, SoLuongTon, OLS_LABEL)
    VALUES (
        p_MaSP, 
        p_TenSP, 
...
-- 3. Tao View phan quyen cho tung loai nguoi dung (Thay the OLS)
CREATE OR REPLACE VIEW VW_SANPHAM_NHANVIEN AS
SELECT MaSP, TenSP, DonGia, LoaiSP, SoLuongTon
FROM SANPHAM
WHERE OLS_LABEL LIKE 'PUB:%';

CREATE OR REPLACE VIEW VW_SANPHAM_QUANLYKHO AS
SELECT MaSP, TenSP, DonGia, LoaiSP, SoLuongTon
FROM SANPHAM
WHERE OLS_LABEL LIKE 'PUB:%' OR OLS_LABEL LIKE 'INT:%';

-- 4. Cap quyen cho cac View
GRANT SELECT ON VW_SANPHAM_NHANVIEN TO ROLE_NHANVIEN_BANHANG;
GRANT SELECT ON VW_SANPHAM_QUANLYKHO TO ROLE_QUANLY_KHO;
GRANT SELECT, INSERT, UPDATE, DELETE ON VW_SANPHAM_QUANLYKHO TO ROLE_QUANLY_KHO;
